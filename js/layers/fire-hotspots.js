...
(function () {
    ...
    var PRODUCT_INFO = {
        'sfide-msg': {
            eyebrow: 'EOSIAL-derived detections',
            title: 'SFIDE - Meteosat Second Generation',
            description: 'SFIDE detections generated by the EOSIAL Laboratory from EUMETSAT MSG SEVIRI Level 1.5 imagery. They are produced with the EOSIAL fire-detection algorithm and are not the operational EUMETSAT fire product.',
            links: [
                { label: 'MSG SEVIRI Level 1.5 data guide', url: 'https://user.eumetsat.int/resources/user-guides/msg-high-rate-seviri-level-1-5-data-guide' }
            ]
        },
        'sfide-mtg': {
            eyebrow: 'EOSIAL-derived detections',
            title: 'SFIDE - Meteosat Third Generation',
            description: 'SFIDE detections generated by the EOSIAL Laboratory from EUMETSAT MTG-I1 FCI Level 1c imagery. They use the EOSIAL fire-detection algorithm rather than the official EUMETSAT MTG-FIR algorithm.',
            links: [
                { label: 'MTG FCI Level 1c data guide', url: 'https://user.eumetsat.int/resources/user-guides/mtg-fci-level-1c-data-guide' }
            ]
        },
        'firms-modis': {
            eyebrow: 'External dataset - NASA FIRMS',
            title: 'MODIS active fire detections',
            description: 'NASA FIRMS near-real-time active fire detections from the MODIS instruments aboard Terra and Aqua. The viewer uses the Collection 6.1 fire products at approximately 1 km nominal resolution.',
            links: [
                { label: 'NASA FIRMS', url: 'https://firms.modaps.eosdis.nasa.gov/' },
                { label: 'MODIS Collection 6 fire guide', url: 'https://www.earthdata.nasa.gov/sites/default/files/imported/MODIS_C6_Fire_User_Guide_B.pdf' }
            ]
        },
        'firms-viirs': {
            eyebrow: 'External dataset - NASA FIRMS',
            title: 'VIIRS active fire detections',
            description: 'NASA FIRMS near-real-time 375 m active fire detections from VIIRS aboard Suomi-NPP, NOAA-20 and NOAA-21. These polar-orbiting observations provide high spatial detail at satellite overpass times.',
            links: [
                { label: 'NASA FIRMS', url: 'https://firms.modaps.eosdis.nasa.gov/' },
                { label: 'VIIRS active fire user guide', url: 'https://www.earthdata.nasa.gov/s3fs-public/imported/VIIRS_activefire_User_Guide.pdf' }
            ]
        },
        's3-standard': {
            eyebrow: 'External dataset - Copernicus Sentinel-3',
            title: 'SLSTR standard FRP detections',
            description: 'Standard 1 km fire detections from the Sentinel-3 SLSTR Level-2 Fire Radiative Power product. The viewer publishes detections from Sentinel-3A and Sentinel-3B as an external comparison dataset.',
            links: [
                { label: 'Sentinel-3 SLSTR products', url: 'https://sentiwiki.copernicus.eu/web/slstr-products' }
            ]
        },
        's3-alternative': {
            eyebrow: 'External dataset - Copernicus Sentinel-3',
            title: 'SLSTR alternative FRP detections',
            description: 'Alternative 1 km detections distributed within the Sentinel-3 SLSTR Level-2 Fire Radiative Power product. They are kept separate from the standard retrieval for direct comparison.',
            links: [
                { label: 'Sentinel-3 SLSTR products', url: 'https://sentiwiki.copernicus.eu/web/slstr-products' }
            ]
        },
        's3-swir': {
            eyebrow: 'External dataset - Copernicus Sentinel-3',
            title: 'SLSTR SWIR detections',
            description: 'The 500 m short-wave infrared detection stream distributed with the Sentinel-3 SLSTR fire product. It is exposed separately because its retrieval and nominal sampling differ from the 1 km thermal streams.',
            links: [
                { label: 'Sentinel-3 SLSTR products', url: 'https://sentiwiki.copernicus.eu/web/slstr-products' },
                { label: 'Sentinel-3 SLSTR instrument', url: 'https://www.esa.int/Applications/Observing_the_Earth/Copernicus/Sentinel-3/Instruments' }
            ]
        },
        'mtg-fir': {
            eyebrow: 'Official external dataset - EUMETSAT',
            title: 'MTG FCI Active Fire Monitoring',
            description: 'The official EUMETSAT MTG-FIR Level-2 Active Fire Monitoring product derived from MTG-I1 FCI observations. It reports fire probability and confidence classes using the operational EUMETSAT algorithm.',
            links: [
                { label: 'MTG FCI Level-2 FIR data guide', url: 'https://user.eumetsat.int/resources/user-guides/mtg-fci-l2-fir-data-guide' },
                { label: 'EUMETSAT fire data resources', url: 'https://user.eumetsat.int/data/themes/emergency/fires' }
            ]
        }
    };
    ...
    function productGroupHtml(id, title, subtitle, listId) {
        return '<fieldset class="fire-product-group" data-product-group="' + id + '">' +
            '<div class="fire-product-heading">' +
            '<label><input type="checkbox" class="fire-product-toggle" checked>' +
            '<span><strong>' + title + '</strong><small>' + subtitle + '</small></span></label>' +
            '<button type="button" class="fire-product-info-btn" data-product-info="' + id +
            '" aria-label="About ' + title + '" title="About this product">i</button></div>' +
            '<div id="' + listId + '" class="fire-product-children"><span class="toolbar-status">Loading...</span></div>' +
            '</fieldset>';
    }

    function closeProductInfo() {
        var modal = document.getElementById('fire-product-info-modal');
        if (modal) modal.classList.add('hidden');
    }

    function showProductInfo(id) {
        var info = PRODUCT_INFO[id];
        if (!info) return;
        var modal = document.getElementById('fire-product-info-modal');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'fire-product-info-modal';
            modal.className = 'fire-product-info-modal hidden';
            modal.setAttribute('role', 'dialog');
            modal.setAttribute('aria-modal', 'true');
            modal.setAttribute('aria-labelledby', 'fire-product-info-title');
            modal.innerHTML =
                '<div class="fire-product-info-panel">' +
                '<button type="button" class="fire-product-info-close" aria-label="Close product information">&times;</button>' +
                '<div class="fire-product-info-eyebrow"></div>' +
                '<h2 id="fire-product-info-title"></h2>' +
                '<p class="fire-product-info-description"></p>' +
                '<div class="fire-product-info-links"></div></div>';
            document.body.appendChild(modal);
            modal.addEventListener('click', function (event) {
                if (event.target === modal || event.target.closest('.fire-product-info-close')) {
                    closeProductInfo();
                }
            });
        }
        modal.querySelector('.fire-product-info-eyebrow').textContent = info.eyebrow;
        modal.querySelector('#fire-product-info-title').textContent = info.title;
        modal.querySelector('.fire-product-info-description').textContent = info.description;
        modal.querySelector('.fire-product-info-links').innerHTML = info.links.map(function (link) {
            return '<a href="' + link.url + '" target="_blank" rel="noopener noreferrer">' +
                link.label + '<span aria-hidden="true"> &nearr;</span></a>';
        }).join('');
        modal.classList.remove('hidden');
        modal.querySelector('.fire-product-info-close').focus();
    }
    ...
    function buildControls(map) {        var container = document.getElementById('product-toolbar-content') || document.getElementById('layer-controls');
        ...
        section.querySelectorAll('.fire-product-info-btn').forEach(function (button) {
            button.addEventListener('click', function (event) {
                event.preventDefault();
                event.stopPropagation();
                showProductInfo(button.getAttribute('data-product-info'));
            });
        });

        document.addEventListener('keydown', function (event) {
            if (event.key === 'Escape') closeProductInfo();
        });
