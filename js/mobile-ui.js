...
(function () {
    ...
    function open(mode) {
        ...
        window.setTimeout(function () {
            var focusable = ui.body.querySelector('button, input, [tabindex="0"]');
    ...
    function handleMediaChange() {
        ...
        updateSummaries();
        if (mapRef) window.setTimeout(function () { mapRef.invalidateSize(); }, 50);
    }
    ...
    function onPointerDown(event) {
        ...
        ui.sheet.classList.add('dragging');
    }

    function onPointerMove(event) {
        if (dragStartY === null) return;
        dragOffset = Math.max(0, event.clientY - dragStartY);
        ui.sheet.style.transform = 'translateY(' + dragOffset + 'px)';
    }

    function onPointerUp(event) {
        if (dragStartY === null) return;
        ui.handle.releasePointerCapture(event.pointerId);
        ui.sheet.classList.remove('dragging');
        dragStartY = null;
        if (dragOffset > 90) close();
        else ui.sheet.style.transform = '';
        dragOffset = 0;
    }
    ...
    function init(map) {
        ...
        ui.handle.addEventListener('pointerdown', onPointerDown);
        ui.handle.addEventListener('pointermove', onPointerMove);
        ui.handle.addEventListener('pointerup', onPointerUp);
        ui.handle.addEventListener('pointercancel', onPointerUp);
        ...
        if (nodes.fire) {
            new MutationObserver(updateSummaries).observe(nodes.fire.element, {
                subtree: true,
                attributes: true,
        ...
        new MutationObserver(function () {
            if (document.body.classList.contains('fire-animation-active')) close();
